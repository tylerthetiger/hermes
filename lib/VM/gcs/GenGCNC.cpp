...
void GenGC::createSnapshot(llvm::raw_ostream &os, bool compact) {
  ...
  auto writeObjectToSnapshot =
      [ptrToOffset, &snap, &visitor, this](const GCCell *cell) {
  ...
  snap.beginRefs();
  youngGen_.forAllObjs(writeObjectToSnapshot);
  oldGen_.forAllObjs(writeObjectToSnapshot);
  snap.endRefs();
